        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
                {DURATION_STATS.map((stat, idx) => (
                    <div key={idx} className="bg-white dark:bg-slate-700/50 rounded-xl border border-gray-200 dark:border-slate-600 p-5">
            ...
            <div className="bg-white dark:bg-slate-700/50 rounded-xl border border-gray-100 dark:border-slate-600 shadow-sm p-6">
                <h3 className="font-bold text-gray-800 dark:text-slate-100 mb-5">Procedure Duration Comparison</h3>
                <div className="space-y-5">
                    {PROCEDURE_DURATIONS.map((proc, idx) => (
                        <div key={idx} className="flex items-center gap-4">
                            <span className="text-sm text-gray-700 dark:text-slate-300 w-40 flex-shrink-0">{proc.name}</span>
                            <div className="flex-1 h-3 bg-gray-100 dark:bg-slate-600 rounded-full overflow-hidden">
                                <div
                            ...
                            <span className="text-sm font-semibold text-gray-700 dark:text-slate-300 w-20 text-right flex-shrink-0">
                                {proc.duration} mins
                            </span>
                        </div>
                    ))}
